...
HeuristicHmax::HeuristicHmax(double discount_factor, const Problem& pb)
...
{
	error.push_function_backtrace("HeuristicHmax::HeuristicHmax");
	throw;
}


double HeuristicHmax::value(const PddlState& st)
{
	...
	std::list<const Action*> candidate_actions;
	std::copy(problem_.actions().begin(), problem_.actions().end(), std::inserter(candidate_actions, candidate_actions.begin()));
	
	while (!candidate_actions.empty())
	{
		bool continue_flag = false;
		
		for (std::list<const Action*>::iterator Ia = candidate_actions.begin() ; Ia != candidate_actions.end() ; ++Ia)
		{
			//double weight = relaxed_hold((*Ia)->precondition());
			double weight = (*Ia)->precondition().hmax_holds(true_atoms_weights_, false_atoms_weights_);
			
			if (weight != std::numeric_limits<unsigned int>::max())
			{
				//continue_flag = relaxed_effect((*Ia)->effect(), weight + 1) || continue_flag;
				continue_flag = ((*Ia)->effect().hmax_relaxation(true_atoms_weights_, false_atoms_weights_, weight + 1)) || continue_flag;
				Ia = candidate_actions.erase(Ia);
			}
			
			//weight = relaxed_hold(problem_.goal());
			weight = problem_.goal().hmax_holds(true_atoms_weights_, false_atoms_weights_);
...
unsigned int HeuristicHmax::relaxed_hold(const StateFormula& formula, bool negate) const
{
	...
	if (conjunction_formula)
	{
		...
		else
		{
			...
			for (FormulaList::const_iterator If = conjunction_formula->conjuncts().begin() ; If != conjunction_formula->conjuncts().end() ; ++If)
				weight = std::max(weight, relaxed_hold(**If, negate));
	...
	throw BaseException(std::string("unexpected state formula ") + typeid(formula).name(), "HeuristicHmax::relaxed_hold");
}


bool HeuristicHmax::relaxed_effect(const Effect& effect, unsigned int weight)
{
	if (effect.empty())
		return false;
	...
	if (add_effect)
	{
		if (true_atoms_weights_[add_effect->atom().index()] == std::numeric_limits<unsigned int>::max())
		{
	...
	if (delete_effect)
	{
		if (false_atoms_weights_[delete_effect->atom().index()] == std::numeric_limits<unsigned int>::max())
		{
	...
	throw BaseException(std::string("unexpected effect ") + typeid(effect).name(), "HeuristicHmax::relaxed_effect");
}




// CLASS GraphHeuristicHmax

template <class Tstate>
GraphHeuristicHmax<Tstate>::GraphHeuristicHmax(GraphMDP<Tstate>& mdp, double discount_factor, const Problem& pb)
try : BaseHeuristic(discount_factor), GraphHeuristic<Tstate>(mdp, discount_factor), HeuristicHmax(discount_factor, pb)
{
}
catch (BaseException& error)
{
	error.push_function_backtrace("GraphHeuristicHmax::GraphHeuristicHmax");
	throw;
}


template <class Tstate>
void GraphHeuristicHmax<Tstate>::compute(Tstate& st)
{
...
INSTANTIATE_STATE_TEMPLATE(GraphHeuristicHmax)




// CLASS SymbolicHeuristicHmax

SymbolicHeuristicHmax::SymbolicHeuristicHmax(SymbolicMDP& mdp, double discount_factor, const Problem& pb)
try : BaseHeuristic(discount_factor), SymbolicHeuristic(mdp, discount_factor), HeuristicHmax(discount_factor, pb)
{
}
catch (BaseException& error)
{
	error.push_function_backtrace("SymbolicHeuristicHmax::SymbolicHeuristicHmax");
	throw;
}


void SymbolicHeuristicHmax::compute(const dd_node_ptr& states)
{
