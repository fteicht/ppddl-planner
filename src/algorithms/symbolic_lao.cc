...
#include "symbolic_lao.h"

SymbolicLAO::SymbolicLAO(const Problem& pb, double epsilon, double discount_factor, unsigned int plan_length, heuristic_t heuristic_type,
...
{
	error.push_function_backtrace("SymbolicLAO::SymbolicLAO");
	throw;
}


SymbolicLAO::~SymbolicLAO()
{
...
void SymbolicLAO::solve_initialize(const PddlState& st)
{
...
void SymbolicLAO::solve_progress()
{
...
bool SymbolicLAO::has_converged()
{
	return (states_frontier_.get() == Cudd_ReadLogicZero(dd_node_ptr::get_cudd_manager()));
}


void SymbolicLAO::compute_backward_reachability()
{
...
void SymbolicLAO::compute_forward_reachability()
{
	forward_reachable_states_.copy(initial_state_);
	dd_node_ptr forward_frontier;
	forward_frontier.copy(initial_state_);
	states_frontier_ = dd_node_ptr(Cudd_ReadLogicZero(dd_node_ptr::get_cudd_manager()));
	policy_deterministic_transitions_ = compute_policy_deterministic_transitions(explored_states_);
	...
	while (forward_frontier.get() != Cudd_ReadLogicZero(dd_node_ptr::get_cudd_manager()))
	{
		...
		forward_frontier = dd_node_ptr(Cudd_bddAnd(dd_node_ptr::get_cudd_manager(), next_states.get(), forward_reachable_states_.get()));
		...
		forward_reachable_states_ = dd_node_ptr(Cudd_bddOr(dd_node_ptr::get_cudd_manager(), forward_reachable_states_.get(), new_states_frontier.get()));

		forward_frontier = dd_node_ptr(Cudd_bddAnd(dd_node_ptr::get_cudd_manager(), new_states_frontier.get(), tip_states_.get()));
		states_frontier_ = dd_node_ptr(Cudd_bddOr(dd_node_ptr::get_cudd_manager(), states_frontier_.get(), forward_frontier.get()));
		forward_frontier = dd_node_ptr(Cudd_bddXor(dd_node_ptr::get_cudd_manager(), new_states_frontier.get(), forward_frontier.get()));
	}

	initialize(states_frontier_);
}
