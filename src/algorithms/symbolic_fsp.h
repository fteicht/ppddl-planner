...
#ifndef SYMBOLIC_FSP_H_
#define SYMBOLIC_FSP_H_

#include "heuristic_algorithm.h"

class SymbolicFSP : public SymbolicHeuristicAlgorithm
{
private :
	dd_node_ptr policy_deterministic_transitions_;
	dd_node_ptr forward_reachable_states_;
	dd_node_ptr backward_reachable_states_;
	dd_node_ptr states_frontier_;

	void compute_forward_reachability();
	void compute_backward_reachability();

public :
	/* Constructs an algorithm */
	SymbolicFSP(const Problem& pb, double epsilon, double discount_factor, unsigned int plan_length, heuristic_t heuristic_type,
