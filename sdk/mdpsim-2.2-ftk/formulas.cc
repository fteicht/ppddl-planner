...
struct Constant : public StateFormula {
  ...
  /* Tests if this state formula holds in the given true and false atom sets */
  virtual bool holds(const AtomSet& true_predicates,
					 const AtomSet& false_predicates,
  ...
  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hadd heuristic (+infinity if not) */
  virtual unsigned int hadd_holds(const std::vector<unsigned int>& true_atoms_weights,
								  const std::vector<unsigned int>& false_atoms_weights,
								  bool negate = false) const {
	if (negate)
		return (value_?(std::numeric_limits<unsigned int>::max()):(0));
	else
		return (value_?(0):(std::numeric_limits<unsigned int>::max()));
  }

  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hmax heuristic (+infinity if not) */
  virtual unsigned int hmax_holds(const std::vector<unsigned int>& true_atoms_weights,
								  const std::vector<unsigned int>& false_atoms_weights,
								  bool negate = false) const {
	if (negate)
		return (value_?(std::numeric_limits<unsigned int>::max()):(0));
...
/* Table of grounded atoms. */
Atom::AtomTable Atom::atoms;

/* Number of grounded non-static atoms (added by Florent Teichteil-Koenigsbuch) */
unsigned int Atom::number_of_atoms_ = 0;
...
const Atom& Atom::make(Predicate predicate, const TermList& terms) {
  ...
  if (!ground) {
	atom->index_ = std::numeric_limits<unsigned int>::max(); // added by Florent Teichteil-Koenigsbuch
    return *atom;
  } else {
    ...
    if (!result.second) {
	  ...
	  // added by Florent Teichteil-Koenigsbuch
	  atom->index_ = number_of_atoms_;
	  number_of_atoms_++;
	  //
      return *atom;
    }
...
/* Tests if this state formula holds in the given true and false atom sets */
bool Atom::holds(const AtomSet& true_predicates,
				 const AtomSet& false_predicates,
				 bool negate) const {
	if (negate)
		return (false_predicates.find(this) == false_predicates.end());
	else
		return (true_predicates.find(this) != true_predicates.end());
}


/* Tests if this state formula holds in the given true and false atoms weights
     for the Hadd heuristic (+infinity if not) */
unsigned int Atom::hadd_holds(const std::vector<unsigned int>& true_atoms_weights,
							  const std::vector<unsigned int>& false_atoms_weights,
							  bool negate) const {
	return (negate?(false_atoms_weights[index_]):(true_atoms_weights[index_]));
}

  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hmax heuristic (+infinity if not) */
unsigned int Atom::hmax_holds(const std::vector<unsigned int>& true_atoms_weights,
							  const std::vector<unsigned int>& false_atoms_weights,
							  bool negate) const {
	return (negate?(false_atoms_weights[index_]):(true_atoms_weights[index_]));
}
...
/* Tests if this state formula holds in the given true and false atom sets */
bool Equality::holds(const AtomSet& true_predicates,
					 const AtomSet& false_predicates,
					 bool negate) const {
	throw std::logic_error("Equality::holds (relaxed) not implemented");
}


/* Tests if this state formula holds in the given true and false atoms weights
     for the Hadd heuristic (+infinity if not) */
unsigned int Equality::hadd_holds(const std::vector<unsigned int>& true_atoms_weights,
								  const std::vector<unsigned int>& false_atoms_weights,
								  bool negate) const {
	throw std::logic_error("Equality::hadd_holds not implemented");
}

  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hmax heuristic (+infinity if not) */
unsigned int Equality::hmax_holds(const std::vector<unsigned int>& true_atoms_weights,
								  const std::vector<unsigned int>& false_atoms_weights,
								  bool negate) const {
	throw std::logic_error("Equality::hmax_holds not implemented");
}
...
/* Tests if this state formula holds in the given true and false atom sets */
bool Comparison::holds(const AtomSet& true_predicates,
					   const AtomSet& false_predicates,
					   bool negate) const {
	throw std::logic_error("Comparison::holds (relaxed) not implemented");
}


/* Tests if this state formula holds in the given true and false atoms weights
     for the Hadd heuristic (+infinity if not) */
unsigned int Comparison::hadd_holds(const std::vector<unsigned int>& true_atoms_weights,
									const std::vector<unsigned int>& false_atoms_weights,
									bool negate) const {
	throw std::logic_error("Comparison::hadd_holds not implemented");
}

  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hmax heuristic (+infinity if not) */
unsigned int Comparison::hmax_holds(const std::vector<unsigned int>& true_atoms_weights,
									const std::vector<unsigned int>& false_atoms_weights,
									bool negate) const {
	throw std::logic_error("Comparison::hmax_holds not implemented");
}
...
/* Tests if this state formula holds in the given true and false atom sets */
bool Negation::holds(const AtomSet& true_predicates,
					 const AtomSet& false_predicates,
					 bool negate) const {
	return (negand_->holds(true_predicates, false_predicates, !negate));
}


/* Tests if this state formula holds in the given true and false atoms weights
     for the Hadd heuristic (+infinity if not) */
unsigned int Negation::hadd_holds(const std::vector<unsigned int>& true_atoms_weights,
								  const std::vector<unsigned int>& false_atoms_weights,
								  bool negate) const {
	return (negand_->hadd_holds(true_atoms_weights, false_atoms_weights, !negate));
}

  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hmax heuristic (+infinity if not) */
unsigned int Negation::hmax_holds(const std::vector<unsigned int>& true_atoms_weights,
								  const std::vector<unsigned int>& false_atoms_weights,
								  bool negate) const {
	return (negand_->hmax_holds(true_atoms_weights, false_atoms_weights, !negate));
}
...
/* Tests if this state formula holds in the given true and false atom sets */
bool Conjunction::holds(const AtomSet& true_predicates,
						const AtomSet& false_predicates,
						bool negate) const {
	if (negate) {
		for (FormulaList::const_iterator If = conjuncts_.begin() ; If != conjuncts_.end() ; ++If) {
			if ((*If)->holds(true_predicates, false_predicates, negate))
				return true;
		}
...
/* Tests if this state formula holds in the given true and false atoms weights
     for the Hadd heuristic (+infinity if not) */
unsigned int Conjunction::hadd_holds(const std::vector<unsigned int>& true_atoms_weights,
									 const std::vector<unsigned int>& false_atoms_weights,
									 bool negate) const {
	if (negate) {
		...
		for (FormulaList::const_iterator If = conjuncts_.begin() ; If != conjuncts_.end() ; ++If)
			weight = std::min(weight, (*If)->hadd_holds(true_atoms_weights, false_atoms_weights, negate));
		...
		for (FormulaList::const_iterator If = conjuncts_.begin() ; If != conjuncts_.end() ; ++If) {
			unsigned int wtp = (*If)->hadd_holds(true_atoms_weights, false_atoms_weights, negate);
  ...
  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hmax heuristic (+infinity if not) */
unsigned int Conjunction::hmax_holds(const std::vector<unsigned int>& true_atoms_weights,
									 const std::vector<unsigned int>& false_atoms_weights,
									 bool negate) const {
	if (negate) {
		unsigned int weight = std::numeric_limits<unsigned int>::max();
		...
		for (FormulaList::const_iterator If = conjuncts_.begin() ; If != conjuncts_.end() ; ++If)
			weight = std::min(weight, (*If)->hmax_holds(true_atoms_weights, false_atoms_weights, negate));
		...
		for (FormulaList::const_iterator If = conjuncts_.begin() ; If != conjuncts_.end() ; ++If)
			weight = std::max(weight, (*If)->hmax_holds(true_atoms_weights, false_atoms_weights, negate));
...
/* Tests if this state formula holds in the given true and false atom sets */
bool Disjunction::holds(const AtomSet& true_predicates,
						const AtomSet& false_predicates,
						bool negate) const {
	if (negate) {
		for (FormulaList::const_iterator If = disjuncts_.begin() ; If != disjuncts_.end() ; ++If) {
			if (!((*If)->holds(true_predicates, false_predicates, negate)))
				return false;
		}
...
/* Tests if this state formula holds in the given true and false atoms weights
     for the Hadd heuristic (+infinity if not) */
unsigned int Disjunction::hadd_holds(const std::vector<unsigned int>& true_atoms_weights,
									 const std::vector<unsigned int>& false_atoms_weights,
									 bool negate) const {
	if (negate) {
		unsigned int weight = 0;
  ...
  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hmax heuristic (+infinity if not) */
unsigned int Disjunction::hmax_holds(const std::vector<unsigned int>& true_atoms_weights,
									 const std::vector<unsigned int>& false_atoms_weights,
									 bool negate) const {
	if (negate) {
		unsigned int weight = 0;
...
/* Tests if this state formula holds in the given true and false atom sets */
bool Quantification::holds(const AtomSet& true_predicates,
						   const AtomSet& false_predicates,
						   bool negate) const {
	throw std::logic_error("Quantification::holds (relaxed) not implemented");
}


/* Tests if this state formula holds in the given true and false atoms weights
     for the Hadd heuristic (+infinity if not) */
unsigned int Quantification::hadd_holds(const std::vector<unsigned int>& true_atoms_weights,
										const std::vector<unsigned int>& false_atoms_weights,
										bool negate) const {
	throw std::logic_error("Quantification::hadd_holds not implemented");
}

  /* Tests if this state formula holds in the given true and false atoms weights
     for the Hmax heuristic (+infinity if not) */
unsigned int Quantification::hmax_holds(const std::vector<unsigned int>& true_atoms_weights,
										const std::vector<unsigned int>& false_atoms_weights,
										bool negate) const {
	throw std::logic_error("Quantification::hmax_holds not implemented");
}
