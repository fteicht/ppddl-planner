...
/* Output operator for terms with type. */
void Term::print_with_type(std::ostream& os) const
{
  if (this->object()) {
    os << TermTable::names_[this->index_];
  } else {
    os << "?v" << -(this->index_);
  }
  extern const Problem* current_problem_hack_;
  if (current_problem_hack_ && (current_problem_hack_->domain().requirements.typing)) {
    os << " - " << TermTable::type(*this);
  }
}
