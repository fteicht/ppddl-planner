...
/* Number of grounded non-static fluents (added by Florent Teichteil-Koenigsbuch) */
unsigned int Fluent::number_of_fluents_ = 0;
...
const Fluent& Fluent::make(const Function& function, const TermList& terms) {
  ...
  if (!ground) {
	fluent->index_ = std::numeric_limits<unsigned int>::max(); // added by Florent Teichteil-Koenigsbuch
    return *fluent;
  } else {
    ...
    if (!result.second) {
	  ...
	  fluent->index_ = number_of_fluents_;
	  number_of_fluents_++;
	  //
      return *fluent;
    }
...
const Fluent* Fluent::fluent(const Function& fun)
{
	for (FluentTable::const_iterator Ifl = fluents.begin() ; Ifl != fluents.end() ; ++Ifl)
	{
		if (((*Ifl)->function()) == fun)
			return (*Ifl);
	}
